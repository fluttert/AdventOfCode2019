...
namespace AdventOfCode2019.Y2019
{
    internal class Orbit
    {
        ...
        public Orbit parent;
        public int orbitLength => parent == null ? 0 : parent.orbitLength + 1;
    ...
    public class Day06 : IAoCDay
    {
        public string SolvePart1(string input = null)
        {
            ...
            // add all orbits (out of order)
            foreach (string line in lines)
            {
                string[] mapInput = line.Split(')');
                if (!orbits.ContainsKey(mapInput[0])) { orbits.Add(mapInput[0], new Orbit(mapInput[0], null)); }
                if (!orbits.ContainsKey(mapInput[1])) { orbits.Add(mapInput[1], new Orbit(mapInput[1], orbits[mapInput[0]])); }
                orbits[mapInput[1]].parent = orbits[mapInput[0]];
            }

            int totalOrbits = 0;
            foreach (var orbit in orbits)
            {
                totalOrbits += orbit.Value.orbitLength;
            }
...
J)K
K)L";
            Debug.Assert(SolvePart1(test1) == "42");
...
K)YOU
I)SAN";
            Debug.Assert(SolvePart2(test2) == "4");
        }

        public string input = @"Z52)127
271)7TV
CJN)HYP
TCQ)65C
2R9)6QM
